...
`retrospective-coach` 是一个用于个人复盘和计划生成的 Codex skill。它会根据你提供的“今天状态”“今天做了什么”“明天想做什么”，生成中文复盘文档或计划文件，并给出理性、具体、带一点活气的改进建议。

它的重点不是替你写一段漂亮总结，而是把事实留下来：你原本说了什么、你实际做了什么、状态如何、哪里值得表扬、哪里应该批评、明天应该用 KISS 怎么改善，以及明天真正要怎么安排。

## 它适合做什么

- 每日复盘：记录当天状态、行动、产出、问题和 KISS 明日行动。
- 每日计划：根据目标、任务、状态和时间生成独立计划文件。
...
KISS 不替代 `原始记录`、`我做了什么`、`今日状态` 和计划目标。它只负责把 AI 的建议压成明天能执行的动作，避免复盘和计划写得热闹但第二天用不上。
...
```text
使用 $retrospective-coach 帮我做明天计划。
状态：明天可能精力一般，上午有 2 小时，晚上能再做 1 小时。
任务：实现计划功能、测试脚本、同步本机、推送 GitHub。
明天想改善：先写验收标准，避免边做边散。
```
...
```powershell
python scripts\retrospective_journal.py --mode plan --plans-root plans --date 2026-06-26 --create
```

同时创建同一天的复盘和计划占位文件：

```powershell
python scripts\retrospective_journal.py --mode both --root reviews --plans-root plans --date 2026-06-26 --create
```
